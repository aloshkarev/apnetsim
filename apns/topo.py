...
from apns.util import irange, natural, naturalSeq
...
class Topo(object):
    ...
    def addWLC(self, name, **opts):
        """Convenience method: Add wlc to graph.
           name: wlc name
           opts: wlc options
           returns: wlc name"""
        if not opts and self.hopts:
            opts = self.hopts
        return self.addNode(name, isWLC=True, **opts)

    def addAP(self, name, **opts):
    ...
    def wlcs(self, sort=True):
        return [n for n in self.nodes(sort) if 'isWLC' in self.g.node[n]]
...
class LinearWirelessTopo(Topo):
    ...
    def build(self, k=2, n=1, **_opts):
        ...
        for i in irange(1, k):
            # Add accessPoint
            ap = self.addAP('ap%s' % i, ssid='ssid_ap%s' % i)
            # Add hosts to accessPoint
            for j in irange(1, n):
                sta = self.addSta(genStaName(i, j))
                self.addLink(sta, ap)
...
class SingleSwitchReversedTopo(Topo):
    """Single switch connected to k hosts, with reversed ports.
       ...
       Useful to verify that Wmnet properly handles custom port
       numberings."""
