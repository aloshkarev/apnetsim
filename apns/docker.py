...
from apns.log import info, error, warn, debug
from apns.node import Host, Node, Station, HostWLC, AP

class Docker(Host):

    def __init__(self, name, dimage=None, dcmd=None, did=None, **kwargs):
        """
        Creates a Docker container as Wmnet host.
    ...
    def start(self):
        # Wmnet ignores the CMD field of the Dockerfile.
        # Lets try to load it here and manually execute it once the
        # container is started and configured by Wmnet:
    ...
    def get_cmd_field(self, imagename):
        ...
        a single /bin/bash command which Wmnet executes
        anyhow.
        """
        try:
...
class DockerSta(Station):
    ...
    def __init__(self, name, dimage="wifi.eltex.loc:5000/sta:latest", dcmd=None, did=None, **kwargs):
        """
        Creates a Docker container as Wmnet host.
        ...
        * updateMemoryLimits(...)
        """
        self.dimage = dimage
        self.dnameprefix = "mn-sta"
        self.dcmd = "runsvdir /etc/sv"
        self.dc = None  # pointer to the dict containing 'Id' and 'Warnings' keys of the container
        self.dcinfo = None
        self.wintfs = {}
        self.wports = {}
        ...
        defaults = {'cpu_quota': -1,
                    'cpu_period': None,
                    'cpu_shares': None,
                    'cpuset_cpus': "1-" + str(os.cpu_count() - 1),
                    'mem_limit': None,
                    'memswap_limit': None,
                    'environment': {},
                    'volumes': [],
                    ...
                    'ipc_mode': None,
                    'devices': [],
                    'cap_add': ['NET_ADMIN', 'SYS_ADMIN'],  # we need this to allow mininet network setup
                    # 'storage_opt': None,
                    'sysctls': {}
                    }
        defaults.update(kwargs)
    ...
    def stop(self, deleteIntfs=True):
        ...
        if deleteIntfs:
            os.system("ovs-docker del-ports ovs-br0 %s.%s" % (self.dnameprefix, self.name))
        container_list = self.dcli.containers(all=True)
        for container in container_list:
            for container_name in container.get("Names", []):
                if "%s.%s" % (self.dnameprefix, self.name) in container_name:
                    self.dcli.remove_container(container="%s.%s" % (self.dnameprefix, self.name), force=True)
                    break
    ...
    def _update_image(self, repository, tag):
        try:
            info('*** Trying to update the image %s %s. \n' % (repository, tag))
            info('*** This can take some minutes...\n')
...
class DockerWLC(HostWLC):

    def __init__(self, name, dimage="wlc:v1.19.x", dcmd=None, did=None, **kwargs):
        """
        Creates a Docker container as Wmnet host.
    ...
    def sendCmd(self, *args, **kwargs):
        ...
        if not self.shell:
            return
        HostWLC.sendCmd(self, *args, **kwargs)
...
class DockerAP(AP):
    ...
    def __init__(self, name, dimage="wifi.eltex.loc:5000/apemu:latest", dcmd=None, did=None, **kwargs):
        """
        Creates a Docker container as Wmnet host.
        ...
        * updateMemoryLimits(...)
        """
        self.dimage = dimage
        self.dnameprefix = "mn-ap"
        self.dcmd = "runsvdir /etc/sv"
        # self.dcmd = "/bin/runsvinit"
        self.dc = None  # pointer to the dict containing 'Id' and 'Warnings' keys of the container
        self.dcinfo = None
        self.wintfs = {}
        self.wan = "eth0"
        ...
        defaults = {'cpu_quota': -1,
                    ...
                    'ipc_mode': None,
                    'devices': [],
                    'cap_add': ['NET_ADMIN', 'SYS_ADMIN', 'SYS_PTRACE'],  # we need this to allow network setup
                    # 'storage_opt': None,
                    'sysctls': {}
                    }
        defaults.update(kwargs)
        ...
        # creats host config for container
        # see: https://docker-py.readthedocs.org/en/latest/hostconfig/
        ap_hc = self.dcli.create_host_config(
            network_mode=self.network_mode,
            privileged=self.privileged,
            binds=self.volumes,
            tmpfs=self.tmpfs,
        ...
        os.system("ovs-docker add-port ovs-br0 %s %s.%s --mtu=1500" % (self.wan, self.dnameprefix, name))
        # fetch information about new container
        self.dcinfo = self.dcli.inspect_container(self.dc)
        self.did = self.dcinfo.get("Id")
    ...
    def stop(self, deleteIntfs=True):
        if deleteIntfs:
            os.system("ovs-docker del-ports ovs-br0 %s.%s" % (self.dnameprefix, self.name))
        self.dcli.stop(container="%s.%s" % (self.dnameprefix, self.name))
        self.dcli.remove_container(container="%s.%s" % (self.dnameprefix, self.name), force=True)
        # for intf in self.intfs:
        #     os.system("aprf -x %s" % (str(self.name) + 'wlan' + str(intf)))
