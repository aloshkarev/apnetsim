...
class simpleTestTopology(unittest.TestCase):
    ...
    def createNet(
        ...
        """
        Creates a Wmnet instance and automatically adds some
        nodes to it.
        """
        self.net = Wmnet(controller=Controller)
        ...
        for i in range(0, ndockers):
            self.d.append(self.net.addDocker('d%d' % i, dimage="ubuntu:22.04"))
    ...
    def getDockerCli(self):
        ...
        if self.docker_cli is None:
            self.docker_cli = docker.api.client.APIClient(
    ...
    def tearDown():
        ...
        with open(os.devnull, 'w') as devnull:
            subprocess.call(
                "docker rm -f $(docker ps --filter 'label=com.mn_docker' -a -q)",
                stdout=devnull,
                stderr=devnull,
                shell=True)
    ...
    def getDockerContainers(self):
        """
        List the containers managed by mn_docker
        """
        return self.getDockerCli().containers(filters={"label": "com.mn_docker"})


# @unittest.skip("disabled connectivity tests for development")
class testDockerConnectivity(simpleTestTopology):
    """
    ...
    def testHostDocker(self):
        ...
        self.assertTrue(len(self.getDockerContainers()) == 1)
    ...
    def testDockerDocker(self):
        ...
        self.startNet()
        # check number of running docker containers
        self.assertTrue(len(self.getDockerContainers()) == 2)
        self.assertTrue(len(self.net.hosts) == 2)
        # check connectivity by using ping
        self.assertTrue(self.net.pingAll() <= 0.0)
        # stop Wmnet network
    ...
    def testDockerMultipleInterfaces(self):
        ...
        self.d.append(self.net.addDocker(
            'd%d' % len(self.d), ip="11.0.0.2", dimage="ubuntu:22.04"))
        # setup links
        ...
        self.startNet()
        # check number of running docker containers
        self.assertTrue(len(self.getDockerContainers()) == 3)
...
class testDockerContainerCommandExecution(simpleTestTopology):
    """
...
class testDockerDynamicTopologies(simpleTestTopology):
    """
    Tests to check dynamic topology support which allows to add
    and remove containers to/from a running Wmnet network instance.
    """
    ...
    def testSimpleAdd(self):
        ...
        d2 = self.net.addDocker('d2', dimage="ubuntu:22.04")
    ...
    def testFullyDynamic(self):
        ...
        ### add container: d4
        d4 = self.net.addDocker('d4', dimage="ubuntu:22.04")
...
class testDockerTCLinks(simpleTestTopology):
    """
...
class testDockerContainerResourceLimitAPI(simpleTestTopology):
    """
    ...
    def testCPUShare(self):
        ...
        # add dockers
        d0 = self.net.addDocker('d0', ip='10.0.0.1', dimage="ubuntu:22.04", cpu_shares=10)
        d1 = self.net.addDocker('d1', ip='10.0.0.2', dimage="ubuntu:22.04", cpu_shares=90)
        # setup links (we always need one connection to suppress warnings)
    ...
    def testCPULimitCFSBased(self):
        ...
        d0 = self.net.addDocker(
            'd0', ip='10.0.0.1', dimage="ubuntu:22.04",
            cpu_period=50000, cpu_quota=10000)
        d1 = self.net.addDocker(
            'd1', ip='10.0.0.2', dimage="ubuntu:22.04",
            cpu_period=50000, cpu_quota=10000)
        # setup links (we always need one connection to suppress warnings)
        self.net.addLink(d0, self.s[0])
        self.net.addLink(d1, self.s[0])
    ...
    @pytest.mark.skipif(os.environ.get("WMNET_NESTED") is not None,
...
# @unittest.skip("disabled container resource limit tests for development")
class testDockerVolumeAPI(simpleTestTopology):
    """
    Test the volume API.
    """
    ...
    def testContainerVolumes(self):
        ...
        self.createNet(nswitches=1, nhosts=0, ndockers=0)
        # add dockers
        d0 = self.net.addDocker('d0', ip='10.0.0.1', dimage="ubuntu:22.04", volumes=["/:/mnt/vol1:rw"])
        d1 = self.net.addDocker('d1', ip='10.0.0.2', dimage="ubuntu:22.04",
                                volumes=["/:/mnt/vol1:rw", "/:/mnt/vol2:rw"])
        # start Wmnet network
        self.startNet()
