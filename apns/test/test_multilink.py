...
validates apns interfaces against systems interfaces
'''
...
class testMultiLink(unittest.TestCase):
    ...
    def testMultiLink(self):
        p = pexpect.spawn('python -m apns.examples.multilink')
        p.expect(self.prompt)
        p.sendline('intfs')
        p.expect('s(\d): lo')
        intfsOutput = p.before
        # parse interfaces from apns intfs, and store them in a list
        ...
        failMsg = ('The systems interfaces and apns interfaces\n'
                   'are not the same')
