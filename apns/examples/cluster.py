...
"""
cluster.py: prototyping/experimentation for distributed Wmnet,
            aka Wmnet: Cluster Edition
...
pair of apns servers that needs to communicate.
...
They are destroyed when the links are deleted in Wmnet.stop()
...
from apns.clean import addCleanupCallback
from apns.examples.clustercli import CLI
from apns.link import Link, Intf
from apns.log import setLogLevel, debug, info, error
from apns.net import Wmnet
from apns.node import Node, Host, OVSSwitch, Controller
from apns.topo import LinearTopo
from apns.topolib import TreeTopo
from apns.util import quietRun, errRun
...
# Some simple placement algorithms for WmnetCluster

class Placer(object):
    "Node placement algorithm for WmnetCluster"
...
# The WmnetCluster class is not strictly necessary.
...
class WmnetCluster(Wmnet):
    "Cluster-enhanced version of Wmnet class"
    ...
    def __init__(self, *args, **kwargs):
        ...
        Wmnet.__init__(self, *args, **params)
    ...
    def modifiedaddHost(self, *args, **kwargs):
        ...
        return Wmnet.addHost(*args, **kwargs)
    ...
    def addController(self, *args, **kwargs):
        ...
        controller = Wmnet.addController(self, *args, **kwargs)
        loopback = '127.0.0.1'
    ...
    def buildFromTopo(self, *args, **kwargs):
        ...
        self.placeNodes()
        info('\n')
        Wmnet.buildFromTopo(self, *args, **kwargs)
...
def testRemoteNet(remote='ubuntu2', link=RemoteGRELink):
    ...
    net = Wmnet(host=RemoteHost, switch=RemoteOVSSwitch, link=link)
    ...
    net.addLink(h2, s2)
    net.start()
    info('Wmnet is running on', quietRun('hostname').strip(), '\n')
...
#
# This shows how existing Wmnet topologies may be used in cluster
...
def testRemoteTopo(link=RemoteGRELink):
    "Test remote Node classes using Wmnet()/Topo() API"
    topo = LinearTopo(2)
    net = Wmnet(topo=topo, host=HostPlacer, switch=SwitchPlacer,
...
def testRemoteSwitches(remote='ubuntu2', link=RemoteGRELink):
    ...
    net = WmnetCluster(topo=topo, servers=servers, link=link,
...
# The WmnetCluster() class integrates pluggable placement
# functions, for maximum ease of use. WmnetCluster() also
# pre-flights and multiplexes server connections.

def testWmnetCluster(remote='ubuntu2', link=RemoteGRELink):
    "Test WmnetCluster()"
    servers = ['localhost', remote]
...
if __name__ == '__main__':
    ...
    # testWmnetCluster( remote=remoteServer, link=remoteLink)
