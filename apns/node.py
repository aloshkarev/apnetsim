...
from apns.link import WirelessIntf, physicalMesh, ITSLink, Intf, TCIntf, OVSIntf, Link
from apns.log import info, error, warn, debug
from apns.moduledeps import moduleDeps, pathCheck, TUN
from apns.util import (errRun, errFail, getincrementaldecoder,
                             quietRun, which, moveIntf, isShellBuiltin,
                             numCores, retry, mountCgroups, BaseString, decode,
                             encode)
from apns.wmediumdConnector import w_server, w_pos, w_cst, wmediumd_mode
...
class Node(object):
    ...
    def __init__(self, name, inNamespace=True, **params):
        ...
        self.waitExited = params.get('waitExited')
    ...
    def setup(cls):
        ...
        pathCheck('mnexec', 'ifconfig', moduleName='Wmnet')
...
class UserSwitch(Switch):
    ...
    @staticmethod
    def TCReapply(intf):
        """Unfortunately user switch and Wmnet are fighting
        ...
        if isinstance(intf, TCIntf):
            ...
            # with new parent, as setup by Wmnet's TCIntf
            parent = res['parent']
...
class OVSSwitch(Switch):
    ...
    @staticmethod
    def TCReapply(intf):
        """Unfortunately OVS and Wmnet are fighting
...
class RemoteController(Controller):
    """Controller running outside of Wmnet's control."""
...
class Node_WiFi(Node):
    ...
    def configLinks(self):
        ...
        from apns.mobility import ConfigMobLinks
        ConfigMobLinks(self)
    ...
    def config(self, mac=None, ip=None, ip6=None,
        ...
        # r = Parent.config( **_params )
        # r = {}
        # if not isinstance(self, Station):
        #     self.setParam(r, 'setMAC', mac=mac)
        # self.setParam(r, 'setIP', ip=ip)
    ...
    def setup(cls):
        """Make sure our class dependencies are available"""
        pathCheck('mnexec', 'ip addr', moduleName='Wmnet')
...
class AP(Node_WiFi):
    """A Switch is a Node that is running (or has execed?)
       an OpenFlow switch."""
    portBase = 0  # Switches start with port 0 in OpenFlow
    dpidLen = 16  # digits in dpid passed to switch
