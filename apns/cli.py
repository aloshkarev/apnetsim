...
from apns.log import info, output, error
from apns.term import makeTerms, runX11
from apns.util import (quietRun, dumpNodeConnections, dumpPorts)

class CLI(Cmd):
    """Simple command-line interface to talk to nodes."""
    prompt = 'apsim> '

    def __init__(self, apns, stdin=sys.stdin, script=None, cmd=None, **kwargs):
        self.cmd = cmd
        if self.cmd:
            self.mn = apns
            self.locals = {'net': apns}
            self.stdin = stdin
            self.inPoller = poll()
    ...
    def initReadline(cls):
        ...
        try:
            ...
            history_path = os.path.expanduser('~/.mn_history.log')
    ...
    helpStr = (
        ...
        'like\n'
        '  apsim> sta2 ping sta3\n'
        'should work.\n'
        '\n'
        'Some character-oriented interactive commands require\n'
        'noecho:\n'
        '  apsim> noecho sta2 nano foo.py\n'
    ...
    def do_time(self, line):
        """Measure time taken for any command in Wmnet."""
        start = time.time()
        self.onecmd(line)
