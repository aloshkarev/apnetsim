...
class PropagationModel(object):
    rssi = -62
    model = 'ITU'  # default propagation model
    exp = 3  # Exponent
    sL = 1  # System Loss
    lF = 12  # Floor penetration loss factor
    pL = 0  # Power Loss Coefficient
    nFloors = 1  # Number of floors
...
class SetSignalRange(object):
    ...
    def logNormalShadowing(self, intf):
        """Log-Normal Shadowing Propagation Loss Model"""
        from apns.wmediumdConnector import WmediumdGRandom, w_server, wmediumd_mode
...
class GetPowerGivenRange(object):
    ...
    def twoRayGround(self, intf):
        """Two Ray Ground Propagation Loss Model (not good result for
        a short distance)"""
        # dist = intf.range
        gt = intf.antennaGain
